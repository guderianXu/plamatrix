...
TEST(SparseOps, cooToCsrReusesOutputStorageForFloat)
{
    ...
    CsrStorage<float, Device::CPU> output(2, 2, 2);
...
TEST(SparseOps, cooToCsrRejectsInvalidInputs)
{
    ...
    CsrStorage<float, Device::CPU> wrong_dimensions(1, 2, 1);
    EXPECT_THROW(
        cooToCsr(2, 2, one_index, one_index, one_value, wrong_dimensions),
        std::runtime_error);

    CsrStorage<float, Device::CPU> wrong_nnz(1, 1, 2);
    EXPECT_THROW(
...
TEST(SparseOps, spmvComputesFloatResultAndReusesOutput)
{
    ...
    DenseStorage<float, Device::CPU> x(4, 1);
    x(0, 0) = 1.0f;
    x(1, 0) = 2.0f;
    x(2, 0) = 3.0f;
    x(3, 0) = 4.0f;
    ...
    DenseStorage<float, Device::CPU> output(3, 1);
    output.fill(-100.0f);
...
TEST(SparseOps, spmvHandlesEmptyRowsAndRejectsDimensionMismatch)
{
    ...
    DenseStorage<double, Device::CPU> x(2, 1);
    x(0, 0) = 3.0;
    x(1, 0) = 4.0;
    ...
    DenseStorage<double, Device::CPU> wrong_rows(3, 1);
    DenseStorage<double, Device::CPU> wrong_columns(2, 2);
    DenseStorage<double, Device::CPU> wrong_output(2, 1);
...
TEST(SparseOps, spmmUsesColumnMajorDenseMatricesAndReusesOutput)
{
    ...
    DenseStorage<double, Device::CPU> B(3, 2);
    B(0, 0) = 1.0;
    B(1, 0) = 2.0;
    B(2, 0) = 3.0;
    B(0, 1) = 10.0;
    ...
    DenseStorage<double, Device::CPU> output(3, 2);
    output.fill(-100.0);
...
TEST(SparseOps, spmmHandlesEmptyMatricesAndRejectsDimensionMismatch)
{
    ...
    DenseStorage<float, Device::CPU> wrong_B(2, 2);
    DenseStorage<float, Device::CPU> wrong_output(1, 2);
...
TEST(SparseOps, spmmRejectsExactDataPointerAliasing)
{
    ...
    DenseStorage<double, Device::CPU> B(2, 2);
...
TEST(CooStorage, cpuToCsrUsesDuplicateCombiningConversion)
{
    CooStorage<CooMatrixScalar, Device::CPU> matrix(2, 3);
...
{
    DenseStorage<Index, Device::CPU> row_cpu(7, 1);
    DenseStorage<Index, Device::CPU> col_cpu(7, 1);
    DenseStorage<TypeParam, Device::CPU> value_cpu(7, 1);
...
{
    auto row_cpu = DenseStorage<Index, Device::CPU>::pinned(5, 1);
    auto col_cpu = DenseStorage<Index, Device::CPU>::pinned(5, 1);
    auto value_cpu = DenseStorage<TypeParam, Device::CPU>::pinned(5, 1);
    ...
    auto row_gpu = DenseStorage<Index, Device::GPU>::uninitializedAsync(5, 1, stream);
    auto col_gpu = DenseStorage<Index, Device::GPU>::uninitializedAsync(5, 1, stream);
    auto value_gpu = DenseStorage<TypeParam, Device::GPU>::uninitializedAsync(5, 1, stream);
    row_cpu.copyToGpuAsync(row_gpu, stream);
    col_cpu.copyToGpuAsync(col_gpu, stream);
    value_cpu.copyToGpuAsync(value_gpu, stream);
    auto output = CsrStorage<TypeParam, Device::GPU>::uninitializedAsync(3, 3, 3, stream);
    SparseOpsWorkspace workspace;
...
{
    DenseStorage<Index, Device::CPU> row_cpu(2, 1);
    DenseStorage<Index, Device::CPU> col_cpu(2, 1);
    DenseStorage<TypeParam, Device::CPU> value_cpu(2, 1);
    row_cpu(0, 0) = 0;
    row_cpu(1, 0) = 1;
    col_cpu(0, 0) = 0;
    col_cpu(1, 0) = 1;
    ...
    DenseStorage<TypeParam, Device::CPU> x_cpu(2, 1);
    x_cpu(0, 0) = TypeParam(4);
    x_cpu(1, 0) = TypeParam(5);
    const auto x_gpu = x_cpu.toGpu();
    DenseStorage<TypeParam, Device::GPU> y_gpu(2, 1);
...
TYPED_TEST(SparseOpsGpuTest, deviceCooToCsrAsyncDefersCoordinateAndNnzErrors)
{
    ...
    const auto col_gpu = col_cpu.toGpu();
    const auto value_gpu = value_cpu.toGpu();
    CsrStorage<TypeParam, Device::GPU> wrong_nnz(2, 2, 2);
    SparseOpsWorkspace workspace;
    ...
    CsrStorage<TypeParam, Device::GPU> valid_capacity(2, 2, 1);
    EXPECT_NO_THROW(cooToCsrAsync(
...
{
    DenseStorage<Index, Device::GPU> no_rows(0, 1);
    DenseStorage<Index, Device::GPU> no_cols(0, 1);
    DenseStorage<TypeParam, Device::GPU> no_values(0, 1);
    CsrStorage<TypeParam, Device::GPU> empty_output(3, 4, 0);
    SparseOpsWorkspace workspace;
...
TYPED_TEST(SparseOpsGpuTest, emptyDeviceCooStatusStillRequiresStreamCompletion)
{
    ...
    DenseStorage<Index, Device::GPU> no_cols(0, 1);
    DenseStorage<TypeParam, Device::GPU> no_values(0, 1);
    CsrStorage<TypeParam, Device::GPU> output(2, 2, 0);
    SparseOpsWorkspace workspace;
    cudaStream_t stream = nullptr;
    ASSERT_EQ(cudaStreamCreateWithFlags(&stream, cudaStreamNonBlocking), cudaSuccess);
...
TYPED_TEST(SparseOpsGpuTest, deviceCooToCsrRejectsCrossStreamOrderedTriplets)
{
    ...
    auto row_gpu = DenseStorage<Index, Device::GPU>::uninitializedAsync(1, 1, owner_stream);
    auto col_gpu = DenseStorage<Index, Device::GPU>::uninitializedAsync(1, 1, owner_stream);
    auto value_gpu = DenseStorage<TypeParam, Device::GPU>::uninitializedAsync(1, 1, owner_stream);
    auto output = CsrStorage<TypeParam, Device::GPU>::uninitializedAsync(1, 1, 1, owner_stream);
    SparseOpsWorkspace workspace;
...
TYPED_TEST(SparseOpsGpuTest, spmvAsyncUsesNonDefaultStreamAndReusesWorkspace)
{
    ...
    DenseStorage<TypeParam, Device::CPU> x_cpu(4, 1);
    ...
    DenseStorage<TypeParam, Device::GPU> output(3, 1);
    SparseOpsWorkspace workspace;
    cudaStream_t stream = nullptr;
    ASSERT_EQ(cudaStreamCreateWithFlags(&stream, cudaStreamNonBlocking), cudaSuccess);
...
{
    auto csr_cpu = CsrStorage<TypeParam, Device::CPU>::pinned(2, 2, 2);
    ...
    CsrStorage<TypeParam, Device::GPU> csr_gpu(2, 2, 2);
    DenseStorage<TypeParam, Device::CPU> input_cpu(2, 1);
    input_cpu.fill(TypeParam(1));
    const auto input_gpu = input_cpu.toGpu();
    DenseStorage<TypeParam, Device::GPU> output_gpu(2, 1);
    SparseOpsWorkspace workspace;
...
TYPED_TEST(SparseOpsGpuTest, synchronousSpmvAllocatesAndReusesOutput)
{
    ...
    DenseStorage<TypeParam, Device::GPU> reused(2, 1);
...
TYPED_TEST(SparseOpsGpuTest, spmmAsyncUsesColumnMajorDenseMatrices)
{
    ...
    DenseStorage<TypeParam, Device::CPU> b_cpu(3, 2);
    ...
    DenseStorage<TypeParam, Device::GPU> output(3, 2);
    SparseOpsWorkspace workspace;
    cudaStream_t stream = nullptr;
    ASSERT_EQ(cudaStreamCreateWithFlags(&stream, cudaStreamNonBlocking), cudaSuccess);
...
TYPED_TEST(SparseOpsGpuTest, synchronousSpmmAllocatesAndReusesOutput)
{
    auto csr_cpu = cooToCsr(
        ...
        std::vector<Index>{0, 1},
        std::vector<TypeParam>{TypeParam(2), TypeParam(3)});
    DenseStorage<TypeParam, Device::CPU> input_cpu(2, 2);
    ...
    DenseStorage<TypeParam, Device::GPU> reused(2, 2);
    TypeParam* pointer = reused.data();
...
TYPED_TEST(SparseOpsGpuTest, productsHandleEmptyMatricesAndRejectInvalidOutputs)
{
    ...
    DenseStorage<TypeParam, Device::CPU> b_cpu(3, 2);
    const auto b_gpu = b_cpu.toGpu();
    DenseStorage<TypeParam, Device::GPU> empty_output(0, 2);
    SparseOpsWorkspace workspace;
    ...
    DenseStorage<TypeParam, Device::CPU> x_cpu(2, 1);
    auto x_gpu = x_cpu.toGpu();
    DenseStorage<TypeParam, Device::GPU> wrong_rows(1, 1);
    DenseStorage<TypeParam, Device::GPU> wrong_columns(2, 2);
...
TYPED_TEST(SparseOpsGpuTest, workspaceSwitchesBetweenMatrixAndVectorDescriptors)
{
    ...
    const auto csr_gpu = csr_cpu.toGpu();
    const auto input_gpu = input_cpu.toGpu();
    DenseStorage<TypeParam, Device::GPU> matrix_output(2, 2);
    DenseStorage<TypeParam, Device::GPU> vector_output(2, 1);
    SparseOpsWorkspace workspace;

    spmmAsync(csr_gpu, input_gpu, matrix_output, workspace);
    DenseStorage<TypeParam, Device::CPU> vector_cpu(2, 1);
...
TYPED_TEST(SparseOpsGpuTest, asyncProductsRejectCrossStreamOrderedStorage)
{
    ...
    csr_cpu.values()[0] = TypeParam(2);
    csr_cpu.values()[1] = TypeParam(3);
    auto input_cpu = DenseStorage<TypeParam, Device::CPU>::pinned(2, 1);
    input_cpu(0, 0) = TypeParam(4);
    input_cpu(1, 0) = TypeParam(5);
    cudaStream_t owner_stream = nullptr;
    cudaStream_t other_stream = nullptr;
    ...
    auto input_gpu = DenseStorage<TypeParam, Device::GPU>::uninitializedAsync(2, 1, owner_stream);
    auto output_gpu = DenseStorage<TypeParam, Device::GPU>::uninitializedAsync(2, 1, owner_stream);
...
TYPED_TEST(SparseOpsGpuTest, workspaceMoveCloseAndCrossStreamRulesAreStable)
{
    ...
    const auto csr_gpu = csr_cpu.toGpu();
    const auto input_gpu = input_cpu.toGpu();
    DenseStorage<TypeParam, Device::GPU> output(2, 1);
...
TYPED_TEST(SparseOpsGpuTest, workspaceRejectsCloseWhileOwnerStreamIsPending)
{
    ...
    DenseStorage<TypeParam, Device::CPU> input_cpu(1, 1);
    input_cpu(0, 0) = TypeParam(3);
    const auto csr_gpu = csr_cpu.toGpu();
    const auto input_gpu = input_cpu.toGpu();
    DenseStorage<TypeParam, Device::GPU> output(1, 1);
    SparseOpsWorkspace workspace;
    cudaStream_t stream = nullptr;
    ASSERT_EQ(cudaStreamCreateWithFlags(&stream, cudaStreamNonBlocking), cudaSuccess);
    std::atomic<bool> release_stream{false};
...
{
    CsrStorage<float, Device::GPU> csr(0, 0, 0);

...
namespace plamatrix::internal
{
    namespace iterative_solver_detail
    {
        ...
        constexpr int kConverged = 8;
        constexpr int kConvergedIteration = 9;
        constexpr int kScalarCount = 10;
        ...
        template <typename Matrix> void checkStreamStorage(const char* name, const Matrix& matrix, cudaStream_t stream)
        {
                            ...
                            bool require_prevalidated_structure)
        {
            ...
            if (!matrix.isStructureUsableOnStream(stream))
            {
                ...
                {
                    throw std::logic_error("fixed CUDA iterative solver requires prevalidated CSR structure; "
    ...
    struct IterativeSolverWorkspaceAccess
    {
        ...
        template <typename Scalar> static SparseOpsWorkspace& sparse(IterativeSolverWorkspace<Scalar>& workspace)
        {
            return workspace._sparseWorkspace;
        }
        template <typename Scalar> static Index size(const IterativeSolverWorkspace<Scalar>& workspace)
        {
            return workspace._capacitySize;
        }

        template <typename Scalar>
        static void bind(IterativeSolverWorkspace<Scalar>& workspace, Index size, cudaStream_t stream)
        {
            if (workspace._hasReuseStream && workspace._reuseStream != stream)
            {
                throw std::logic_error("IterativeSolverWorkspace cannot be reused on another stream; close it first");
            }
        ...
        template <typename Scalar>
        static cublasHandle_t handle(IterativeSolverWorkspace<Scalar>& workspace, cudaStream_t stream)
        {
    ...
    struct IterativeSolverStateAccess
    {
        ...
        {
            PLAMATRIX_CHECK_CUDA(cudaEventRecord(static_cast<cudaEvent_t>(state._completionEvent), state._stream));
        }
    };

...
namespace plamatrix
{
    ...
    namespace
    {
                        ...
                        cudaStream_t stream)
        {
            ...
            PLAMATRIX_CHECK_CUDA(
                cudaMemsetAsync(scalars.data(), 0, static_cast<std::size_t>(kScalarCount) * sizeof(Scalar), stream));
            spmvAsync(matrix, solution, matrix_direction, IterativeSolverWorkspaceAccess::sparse(workspace), stream);
            ...
            else if (preconditioned)
            {
                jacobiKernel<<<gridSize(size), kBlockSize, 0, stream>>>(
                    ...
                    matrix.colIndices(),
                    matrix.values(),
                    IterativeSolverWorkspaceAccess::inverseDiagonal(workspace).data(),
                    size);
            ...
            else
            {
                PLAMATRIX_CHECK_CUBLAS(
                    Blas<Scalar>::copy(handle, static_cast<int>(size), residual.data(), transformed.data()));
            }
            PLAMATRIX_CHECK_CUBLAS(
                Blas<Scalar>::copy(handle, static_cast<int>(size), transformed.data(), direction.data()));
            PLAMATRIX_CHECK_CUBLAS(Blas<Scalar>::dot(
                handle, static_cast<int>(size), residual.data(), transformed.data(), scalars.data() + kRho));
        }
                        ...
                        cudaStream_t stream)
        {
            ...
            auto& residual = IterativeSolverWorkspaceAccess::residual(workspace);
            auto& scalars = IterativeSolverWorkspaceAccess::scalars(workspace);
            spmvAsync(matrix, direction, matrix_direction, IterativeSolverWorkspaceAccess::sparse(workspace), stream);
            PLAMATRIX_CHECK_CUBLAS(Blas<Scalar>::dot(
                handle, size, direction.data(), matrix_direction.data(), scalars.data() + kDenominator));
            alphaKernel<Scalar><<<1, 1, 0, stream>>>(scalars.data());
            PLAMATRIX_CHECK_CUDA(cudaGetLastError());
            updateSolutionAndResidualKernel<Scalar><<<gridSize(size), kBlockSize, 0, stream>>>(
                scalars.data(), direction.data(), matrix_direction.data(), solution.data(), residual.data(), size);
            PLAMATRIX_CHECK_CUDA(cudaGetLastError());
        }
                             ...
                             cudaStream_t stream)
        {
            ...
            {
                PLAMATRIX_CHECK_CUBLAS(Blas<Scalar>::copy(handle, size, residual.data(), transformed.data()));
            }
            PLAMATRIX_CHECK_CUBLAS(
                Blas<Scalar>::dot(handle, size, residual.data(), transformed.data(), scalars.data() + kNextRho));
            betaKernel<Scalar><<<1, 1, 0, stream>>>(scalars.data());
            PLAMATRIX_CHECK_CUDA(cudaGetLastError());
            updateDirectionKernel<Scalar>
                <<<gridSize(size), kBlockSize, 0, stream>>>(scalars.data(), transformed.data(), direction.data(), size);
            PLAMATRIX_CHECK_CUDA(cudaGetLastError());
        }
        ...
        void residualSquared(IterativeSolverWorkspace<Scalar>& workspace, cudaStream_t stream)
        {
            ...
            auto& residual = IterativeSolverWorkspaceAccess::residual(workspace);
            auto& scalars = IterativeSolverWorkspaceAccess::scalars(workspace);
            PLAMATRIX_CHECK_CUBLAS(Blas<Scalar>::dot(handle,
                                                     static_cast<int>(IterativeSolverWorkspaceAccess::size(workspace)),
                                                     residual.data(),
                                                     residual.data(),
                                                     scalars.data() + kResidualSquared));
        }
                                             ...
                                             bool preconditioned)
        {
            ...
            if (matrix.rows() == 0)
            {
                PLAMATRIX_CHECK_CUDA(cudaMemsetAsync(state.initialResidualSquared.data(), 0, sizeof(double), stream));
                PLAMATRIX_CHECK_CUDA(cudaMemsetAsync(state.finalResidualSquared.data(), 0, sizeof(double), stream));
            }
            else
            {
                residualSquared(workspace, stream);
                toDoubleKernel<<<1, 1, 0, stream>>>(IterativeSolverWorkspaceAccess::scalars(workspace).data() +
                                                        kResidualSquared,
            ...
            {
                waitForFixedSolverCompletionGate<<<1, 1, 0, stream>>>(static_cast<int*>(fixed_solver_completion_gate));
                PLAMATRIX_CHECK_CUDA(cudaGetLastError());
            }
#endif
                                         ...
                                         cudaStream_t stream)
        {
            if (block_size <= 0 || matrix.rows() % block_size != 0 ||
                (matrix.rows() > 0 && block_size > std::numeric_limits<Index>::max() / matrix.rows()) ||
                inverse_blocks.rows() != matrix.rows() * block_size || inverse_blocks.cols() != 1)
            {
                throw std::invalid_argument("CUDA block PCG requires complete row-major inverse diagonal blocks");
            }
            if (inverse_blocks.data() != nullptr &&
                (inverse_blocks.data() == rhs.data() || inverse_blocks.data() == solution.data()))
            {
                throw std::invalid_argument("CUDA block PCG inverse blocks must not alias rhs or solution");
            }
                                            ...
                                            Index block_size = 0)
        {
            ...
            initialize(matrix, rhs, solution, workspace, preconditioned, inverse_blocks, block_size, stream);
            ...
            const double maximum_tolerance_squared = static_cast<double>(std::numeric_limits<Scalar>::max());
            const Scalar tolerance_squared = static_cast<Scalar>(
                tolerance >= std::sqrt(maximum_tolerance_squared) ? maximum_tolerance_squared : tolerance * tolerance);
            constexpr std::size_t status_scalar_count = kScalarCount - kResidualSquared;
            std::array<Scalar, status_scalar_count> host_status{};
            for (int iteration = 0; iteration < options.maxIterations; ++iteration)
            {
                submitStep(matrix, solution, workspace, stream);
                residualSquared(workspace, stream);
                convergenceKernel<Scalar><<<1, 1, 0, stream>>>(
                    IterativeSolverWorkspaceAccess::scalars(workspace).data(), tolerance_squared, iteration + 1);
                PLAMATRIX_CHECK_CUDA(cudaGetLastError());
                const bool check_status =
                    (iteration + 1) % options.convergenceCheckInterval == 0 || iteration + 1 == options.maxIterations;
                if (check_status)
                {
                    PLAMATRIX_CHECK_CUDA(
                        cudaMemcpyAsync(host_status.data(),
                                        IterativeSolverWorkspaceAccess::scalars(workspace).data() + kResidualSquared,
                                        host_status.size() * sizeof(Scalar),
                                        cudaMemcpyDeviceToHost,
                                        stream));
                    PLAMATRIX_CHECK_CUDA(cudaStreamSynchronize(stream));
                    host_squared = host_status[0];
                    host_breakdown = host_status[1];
                    if (host_breakdown != Scalar{0})
                    {
                        throw std::runtime_error("CUDA iterative solver breakdown: matrix is not numerically SPD");
                    }
                    if (!std::isfinite(static_cast<double>(host_squared)) || host_squared < Scalar{0})
                    {
                        throw std::runtime_error("CUDA iterative solver breakdown: matrix is not numerically SPD");
                    ...
                    if (host_status[2] != Scalar{0})
                    {
                        const double converged_iteration = static_cast<double>(host_status[3]);
                        if (!std::isfinite(converged_iteration) || converged_iteration < 1.0 ||
                            converged_iteration > static_cast<double>(iteration + 1))
                        {
                            throw std::runtime_error("CUDA iterative solver produced an invalid convergence iteration");
                        }
                        report.iterations = static_cast<int>(converged_iteration);
                        report.converged = true;
                        break;
                    }
                }
                ...
                {
                    updateDirection(workspace, preconditioned, inverse_blocks, block_size, stream);
                }
            }
    ...
    IterativeSolverReport pcg(const CSRMatrix<Scalar, Device::GPU>& matrix,
                              ...
                              cudaStream_t stream)
    {
        return adaptiveSolve(matrix, rhs, solution, workspace, options, stream, options.useJacobiPreconditioner);
    }
    ...
    IterativeSolverReport blockPcg(const CSRMatrix<Scalar, Device::GPU>& matrix,
                                   ...
                                   cudaStream_t stream)
    {
        validateBlockPreconditioner(matrix, rhs, solution, inverse_blocks, block_size, stream);
        return adaptiveSolve(matrix, rhs, solution, workspace, options, stream, true, &inverse_blocks, block_size);
    }
                                                        ...
                                                        const IterativeSolverOptions& options)
    {
        ...
        {
            throw std::logic_error("finalizeIterativeSolverReport requires caller stream synchronization");
        }
        PLAMATRIX_CHECK_CUDA(status);
        double initial_squared = 0.0;
        double final_squared = 0.0;
        PLAMATRIX_CHECK_CUDA(cudaMemcpyAsync(&initial_squared,
                                             state.initialResidualSquared.data(),
                                             sizeof(double),
                                             cudaMemcpyDeviceToHost,
                                             state._stream));
        PLAMATRIX_CHECK_CUDA(cudaMemcpyAsync(
            &final_squared, state.finalResidualSquared.data(), sizeof(double), cudaMemcpyDeviceToHost, state._stream));
        PLAMATRIX_CHECK_CUDA(cudaStreamSynchronize(state._stream));
        if (!std::isfinite(initial_squared) || !std::isfinite(final_squared) || initial_squared < 0.0 ||
            final_squared < 0.0)
        {
            throw std::runtime_error("fixed CUDA iterative solver breakdown: residual is not finite");
        }
        IterativeSolverReport report;

...
namespace plamatrix::internal::block_schur_detail
{
    ...
    struct SchurComplementSolverWorkspaceAccess
    {
                            ...
                            const std::vector<Index>& topology)
        {
            return workspace._primaryBlockCount == primary_count &&
                   workspace._eliminatedBlockCount == eliminated_count && workspace._primaryBlockSize == primary_size &&
                   workspace._eliminatedBlockSize == eliminated_size && workspace._topology == topology &&
                            ...
                            std::vector<Index> topology)
        {
            ...
            struct BlockKeyHash
            {
                ...
                {
                    const std::size_t first = std::hash<Index>{}(key.first);
                    const std::size_t second = std::hash<Index>{}(key.second);
                    return first ^ (second + 0x9e3779b9U + (first << 6U) + (first >> 2U));
                }
            };
            std::unordered_map<BlockKey, Index, BlockKeyHash> slot_by_key;
            const auto primary_size_count = static_cast<std::size_t>(primary_count);
            if (primary_size_count > 0 &&
                primary_size_count <= std::numeric_limits<std::size_t>::max() / primary_size_count)
            {
                slot_by_key.reserve(std::min<std::size_t>(primary_size_count * primary_size_count, 1U << 20U));
            }
            ...
            for (Index eliminated = 0; eliminated < eliminated_count; ++eliminated)
            {
                ...
                for (const std::size_t left_index : entries)
                {
                    ...
                    {
                        slot_by_key.emplace(
                            BlockKey{cross_blocks[left_index].primaryBlock, cross_blocks[right_index].primaryBlock}, 0);
                    }
                }
            }

            std::vector<BlockKey> sorted_keys;
            sorted_keys.reserve(slot_by_key.size());
            for (const auto& [key, value] : slot_by_key)
            {
            ...
            for (const BlockKey& key : sorted_keys)
            {
                ...
                slot_by_key.at(key) = slot++;
                workspace._blockSlots.push_back({key.first, key.second, column_ordinal++});
            }

            workspace._diagonalSlots.resize(static_cast<std::size_t>(primary_count));
            for (Index block = 0; block < primary_count; ++block)
            {
                workspace._diagonalSlots[static_cast<std::size_t>(block)] = slot_by_key.at(BlockKey{block, block});
            }
            workspace._primaryPairSlots.clear();
            workspace._primaryPairSlots.reserve(primary_cross_blocks.size() * 2);
            for (const auto& cross : primary_cross_blocks)
            {
                workspace._primaryPairSlots.push_back(slot_by_key.at(BlockKey{cross.rowBlock, cross.columnBlock}));
                workspace._primaryPairSlots.push_back(slot_by_key.at(BlockKey{cross.columnBlock, cross.rowBlock}));
            }
            ...
            {
                const auto& entries = adjacency[static_cast<std::size_t>(eliminated)];
                auto& pair_slots = workspace._eliminatedPairSlots[static_cast<std::size_t>(eliminated)];
                ...
                for (const std::size_t left_index : entries)
                {
                    for (const std::size_t right_index : entries)
                    {
                        pair_slots.push_back(slot_by_key.at(
                            BlockKey{cross_blocks[left_index].primaryBlock, cross_blocks[right_index].primaryBlock}));
                    }
                }
            }
            ...
            for (Index block_row = 0; block_row < primary_count; ++block_row)
            {
                for (Index local_row = 0; local_row < primary_size; ++local_row)
                {
                    ...
                    for (const auto& block_slot : workspace._blockSlots)
                    {
                        ...
                        {
                            workspace._columnIndices.push_back(block_slot.blockColumn * primary_size + column);
                            ++cursor;
                        }
                    }
                }
            ...
            for (std::size_t block_slot = 0; block_slot < workspace._blockSlots.size(); ++block_slot)
            {
                for (Index row = 0; row < primary_size; ++row)
                {
                    for (Index column = 0; column < primary_size; ++column)
                    {
                        const Index position = valuePosition(workspace, static_cast<Index>(block_slot), row, column);
                        workspace._valueBlockSlots[static_cast<std::size_t>(position)] = static_cast<Index>(block_slot);
            ...
            for (Index block = 0; block < primary_count; ++block)
            {
                ...
                for (Index row = 0; row < primary_size; ++row)
                {
                    for (Index column = 0; column < primary_size; ++column)
                    {
                        const Index position = valuePosition(workspace, diagonal_slot, row, column);
            ...
            for (std::size_t cross_index = 0; cross_index < primary_cross_blocks.size(); ++cross_index)
            {
                ...
                for (Index row = 0; row < primary_size; ++row)
                {
                    ...
                    {
                        const Index source =
                            static_cast<Index>(cross_index) * primary_size * primary_size + row * primary_size + column;
                        const Index forward = valuePosition(workspace, forward_slot, row, column);
                        const Index transpose = valuePosition(workspace, transpose_slot, column, row);
            ...
            workspace._slotTermOffsets.assign(workspace._blockSlots.size() + 1, 0);
            for (Index eliminated = 0; eliminated < eliminated_count; ++eliminated)
            {
                const auto& entries = adjacency[static_cast<std::size_t>(eliminated)];
                const auto& pair_slots = workspace._eliminatedPairSlots[static_cast<std::size_t>(eliminated)];
                for (std::size_t left = 0; left < entries.size(); ++left)
                {
                    for (std::size_t right = 0; right < entries.size(); ++right)
                    {
                        ...
                        ++workspace._slotTermOffsets[static_cast<std::size_t>(pair_slot + 1)];
                    }
                }
            }
            for (std::size_t slot = 0; slot < workspace._blockSlots.size(); ++slot)
            {
                workspace._slotTermOffsets[slot + 1] += workspace._slotTermOffsets[slot];
            }
            const std::size_t term_count = static_cast<std::size_t>(workspace._slotTermOffsets.back());
            workspace._slotTermEliminated.resize(term_count);
            workspace._slotTermLeftCross.resize(term_count);
            workspace._slotTermRightCross.resize(term_count);
            std::vector<Index> write_offsets = workspace._slotTermOffsets;
            for (Index eliminated = 0; eliminated < eliminated_count; ++eliminated)
            {
                const auto& entries = adjacency[static_cast<std::size_t>(eliminated)];
                const auto& pair_slots = workspace._eliminatedPairSlots[static_cast<std::size_t>(eliminated)];
                for (std::size_t left = 0; left < entries.size(); ++left)
                {
                    for (std::size_t right = 0; right < entries.size(); ++right)
                    {
                        ...
                        const std::size_t position =
                            static_cast<std::size_t>(write_offsets[static_cast<std::size_t>(pair_slot)]++);
                        workspace._slotTermEliminated[position] = eliminated;
                        workspace._slotTermLeftCross[position] = static_cast<Index>(entries[left]);
                        workspace._slotTermRightCross[position] = static_cast<Index>(entries[right]);
                    }
                }
            }
        ...
        template <typename Scalar>
        static const std::vector<Index>& rowOffsets(const SchurComplementSolverWorkspace<Scalar>& workspace)
        {
            return workspace._rowOffsets;
        }

        template <typename Scalar>
        static const std::vector<Index>& columnIndices(const SchurComplementSolverWorkspace<Scalar>& workspace)
        {
            return workspace._columnIndices;
        }

        template <typename Scalar>
        static Index diagonalSlot(const SchurComplementSolverWorkspace<Scalar>& workspace, Index block)
        {
            return workspace._diagonalSlots[static_cast<std::size_t>(block)];
        }

        template <typename Scalar>
        static const std::vector<Index>& eliminatedPairSlots(const SchurComplementSolverWorkspace<Scalar>& workspace,
                                                             Index eliminated)
        {
        ...
        template <typename Scalar>
        static const std::vector<Index>& primaryPairSlots(const SchurComplementSolverWorkspace<Scalar>& workspace)
        {
            return workspace._primaryPairSlots;
        }

        template <typename Scalar>
        static const std::vector<Index>& valueBaseKinds(const SchurComplementSolverWorkspace<Scalar>& workspace)
        {
            return workspace._valueBaseKinds;
        }

        template <typename Scalar>
        static const std::vector<Index>& valueBaseIndices(const SchurComplementSolverWorkspace<Scalar>& workspace)
        {
            return workspace._valueBaseIndices;
        }

        template <typename Scalar>
        static const std::vector<Index>& valueLocalRows(const SchurComplementSolverWorkspace<Scalar>& workspace)
        {
            return workspace._valueLocalRows;
        }

        template <typename Scalar>
        static const std::vector<Index>& valueLocalColumns(const SchurComplementSolverWorkspace<Scalar>& workspace)
        {
            return workspace._valueLocalColumns;
        }

        template <typename Scalar>
        static const std::vector<Index>& valueBlockSlots(const SchurComplementSolverWorkspace<Scalar>& workspace)
        {
            return workspace._valueBlockSlots;
        }

        template <typename Scalar>
        static const std::vector<Index>& slotTermOffsets(const SchurComplementSolverWorkspace<Scalar>& workspace)
        {
            return workspace._slotTermOffsets;
        }

        template <typename Scalar>
        static const std::vector<Index>& slotTermEliminated(const SchurComplementSolverWorkspace<Scalar>& workspace)
        {
            return workspace._slotTermEliminated;
        }

        template <typename Scalar>
        static const std::vector<Index>& slotTermLeftCross(const SchurComplementSolverWorkspace<Scalar>& workspace)
        {
            return workspace._slotTermLeftCross;
        }

        template <typename Scalar>
        static const std::vector<Index>& slotTermRightCross(const SchurComplementSolverWorkspace<Scalar>& workspace)
        {
            return workspace._slotTermRightCross;
        }

        template <typename Scalar>
        static std::size_t blockSlotCount(const SchurComplementSolverWorkspace<Scalar>& workspace)
        {
            return workspace._blockSlots.size();
        }

        template <typename Scalar>
        static Index blockSlotRow(const SchurComplementSolverWorkspace<Scalar>& workspace, std::size_t slot)
        {
            return workspace._blockSlots[slot].blockRow;
        }

        template <typename Scalar>
        static Index blockSlotColumn(const SchurComplementSolverWorkspace<Scalar>& workspace, std::size_t slot)
        {
            return workspace._blockSlots[slot].blockColumn;
        }

        template <typename Scalar>
        static std::vector<Scalar>& hostDenseSchur(SchurComplementSolverWorkspace<Scalar>& workspace)
        {
            return workspace._hostDenseSchur;
        }

        template <typename Scalar>
        static std::vector<Scalar>& hostDenseReference(SchurComplementSolverWorkspace<Scalar>& workspace)
        {
            return workspace._hostDenseReference;
        }

        template <typename Scalar>
        static std::vector<Scalar>& hostTransformedCross(SchurComplementSolverWorkspace<Scalar>& workspace)
        {
            return workspace._hostTransformedCross;
        }

        template <typename Scalar>
        static std::vector<Scalar>& hostPrimaryDiagonal(SchurComplementSolverWorkspace<Scalar>& workspace)
        {
            return workspace._hostPrimaryDiagonal;
        }

        template <typename Scalar>
        static std::vector<Scalar>& hostEliminatedDiagonal(SchurComplementSolverWorkspace<Scalar>& workspace)
        {
            return workspace._hostEliminatedDiagonal;
        }

        template <typename Scalar>
        static std::vector<Scalar>& hostEliminatedInverse(SchurComplementSolverWorkspace<Scalar>& workspace)
        {
            return workspace._hostEliminatedInverse;
        }

        template <typename Scalar>
        static std::vector<Scalar>& hostReducedRhs(SchurComplementSolverWorkspace<Scalar>& workspace)
        {
            return workspace._hostReducedRhs;
        }

        template <typename Scalar>
        static std::vector<Scalar>& hostScratch(SchurComplementSolverWorkspace<Scalar>& workspace)
        {
        ...
        template <typename Scalar>
        static std::shared_ptr<void>& mixedPrecisionState(SchurComplementSolverWorkspace<Scalar>& workspace)
        {
            return workspace._mixedPrecisionState;
        }

        template <typename Scalar>
        static std::shared_ptr<void>& deviceAssemblyState(SchurComplementSolverWorkspace<Scalar>& workspace)
        {
    ...
    {
        std::vector<Index> topology{primary_count, eliminated_count, primary_size, eliminated_size};
        ...
        const bool reused = SchurComplementSolverWorkspaceAccess::matches(
            workspace, primary_count, eliminated_count, primary_size, eliminated_size, topology);
        if (!reused)
        {
            SchurComplementSolverWorkspaceAccess::rebuild(workspace,
                                                          primary_count,
                                                          eliminated_count,
                                                          primary_size,
                                                          eliminated_size,
    ...
    template <typename Scalar, typename PrimaryCrossBlocks, typename CrossBlocks, typename Adjacency>
    void assembleReducedSchurDenseLower(Index primary_count,
                                        Index eliminated_count,
                                        Index primary_size,
                                        Index eliminated_size,
                                        ...
                                        std::vector<Scalar>** dense_reference)
    {
        ...
        auto& transformed_cross = SchurComplementSolverWorkspaceAccess::hostTransformedCross(workspace);
        dense.assign(static_cast<std::size_t>(dimension * dimension), Scalar(0));
        transformed_cross.resize(cross_blocks.size() * static_cast<std::size_t>(primary_size * eliminated_size));
...
#pragma omp parallel for schedule(static) if (cross_count >= 64 && !omp_in_parallel())
        for (Index cross_index = 0; cross_index < cross_count; ++cross_index)
        {
            ...
            const Scalar* inverse = eliminated_inverse.data() +
                                    static_cast<std::size_t>(cross.eliminatedBlock * eliminated_size * eliminated_size);
            Scalar* transformed =
                transformed_cross.data() + static_cast<std::size_t>(cross_index * primary_size * eliminated_size);
        ...
        const auto& term_left_cross = SchurComplementSolverWorkspaceAccess::slotTermLeftCross(workspace);
        const auto& term_right_cross = SchurComplementSolverWorkspaceAccess::slotTermRightCross(workspace);
        const Index slot_count = static_cast<Index>(SchurComplementSolverWorkspaceAccess::blockSlotCount(workspace));
#pragma omp parallel for schedule(static) if (slot_count >= 32 && !omp_in_parallel())
        for (Index slot = 0; slot < slot_count; ++slot)
        {
            const Index block_row =
                SchurComplementSolverWorkspaceAccess::blockSlotRow(workspace, static_cast<std::size_t>(slot));
            const Index block_column =
                SchurComplementSolverWorkspaceAccess::blockSlotColumn(workspace, static_cast<std::size_t>(slot));
            ...
            for (Index local_row = 0; local_row < primary_size; ++local_row)
            {
                const Index local_column_end = block_row == block_column ? local_row + 1 : primary_size;
                for (Index local_column = 0; local_column < local_column_end; ++local_column)
                {
                    const Index value_position =
                        SchurComplementSolverWorkspaceAccess::valuePosition(workspace, slot, local_row, local_column);
                    ...
                    for (Index term = term_offsets[static_cast<std::size_t>(slot)];
                         ...
                         ++term)
                    {
                        const Index left_index = term_left_cross[static_cast<std::size_t>(term)];
                        const Index right_index = term_right_cross[static_cast<std::size_t>(term)];
                        const Scalar* transformed =
                            transformed_cross.data() +
                            static_cast<std::size_t>(left_index * primary_size * eliminated_size +
                                                     local_row * eliminated_size);
                        const Scalar* right = cross_blocks[static_cast<std::size_t>(right_index)].values.data() +
                                              local_column * eliminated_size;
                        Scalar product = eliminated_size == 3 ? dot3(transformed, right) : Scalar(0);
...
#pragma omp simd reduction(+ : product)
        ...
        if (accumulation_seconds)
        {
            *accumulation_seconds =
                std::chrono::duration<double>(std::chrono::steady_clock::now() - accumulation_start).count();
        }
        *dense_lower = &dense;
                                                            ...
                                                            double* csr_conversion_seconds = nullptr)
    {
        ...
        const auto& column_indices = SchurComplementSolverWorkspaceAccess::columnIndices(workspace);
        if (column_indices.size() > static_cast<std::size_t>(std::numeric_limits<Index>::max()))
        {
        ...
        if (csr_conversion_seconds)
        {
            *csr_conversion_seconds =
                std::chrono::duration<double>(std::chrono::steady_clock::now() - conversion_start).count();
        }
        const auto accumulation_start = std::chrono::steady_clock::now();
        ...
        if (device_assembly)
        {
            ...
            for (const auto& cross : primary_cross_blocks)
            {
                flattened_primary_cross.insert(flattened_primary_cross.end(), cross.values.begin(), cross.values.end());
            }
            std::vector<Scalar> flattened_cross;
            flattened_cross.reserve(cross_blocks.size() * static_cast<std::size_t>(primary_size * eliminated_size));
            std::vector<Index> cross_eliminated_blocks;
            cross_eliminated_blocks.reserve(cross_blocks.size());
            for (const auto& cross : cross_blocks)
            {
                flattened_cross.insert(flattened_cross.end(), cross.values.begin(), cross.values.end());
                cross_eliminated_blocks.push_back(cross.eliminatedBlock);
            }
            const auto& base_kinds = SchurComplementSolverWorkspaceAccess::valueBaseKinds(workspace);
            const auto& base_indices = SchurComplementSolverWorkspaceAccess::valueBaseIndices(workspace);
            const auto& value_block_slots = SchurComplementSolverWorkspaceAccess::valueBlockSlots(workspace);
            const auto& local_rows = SchurComplementSolverWorkspaceAccess::valueLocalRows(workspace);
            const auto& local_columns = SchurComplementSolverWorkspaceAccess::valueLocalColumns(workspace);
            const auto& term_offsets = SchurComplementSolverWorkspaceAccess::slotTermOffsets(workspace);
            const auto& term_eliminated = SchurComplementSolverWorkspaceAccess::slotTermEliminated(workspace);
            const auto& term_left_cross = SchurComplementSolverWorkspaceAccess::slotTermLeftCross(workspace);
            const auto& term_right_cross = SchurComplementSolverWorkspaceAccess::slotTermRightCross(workspace);
            std::vector<Scalar> values;
            if (backend == SchurComplementLinearBackend::Cuda)
            {
                values = assembleSchurValuesOnCuda(primary_size,
                                                   eliminated_size,
                                                   primary_diagonal,
                                                   eliminated_inverse,
        ...
        const auto add_value = [&](Index slot, Index row, Index column, Scalar value)
        { matrix_values[SchurComplementSolverWorkspaceAccess::valuePosition(workspace, slot, row, column)] += value; };
        for (Index block = 0; block < primary_count; ++block)
        {
            const Index slot = SchurComplementSolverWorkspaceAccess::diagonalSlot(workspace, block);
        ...
        const auto& primary_pair_slots = SchurComplementSolverWorkspaceAccess::primaryPairSlots(workspace);
        for (std::size_t index = 0; index < primary_cross_blocks.size(); ++index)
        {
            ...
            for (Index row = 0; row < primary_size; ++row)
            {
                for (Index column = 0; column < primary_size; ++column)
                {
                    const Scalar value = cross.values[static_cast<std::size_t>(row * primary_size + column)];
                    add_value(forward_slot, row, column, value);
                    add_value(transpose_slot, column, row, value);
                }
            }
        }

        auto& transformed = SchurComplementSolverWorkspaceAccess::hostTransformedCross(workspace);
        const std::size_t transformed_block_size = static_cast<std::size_t>(primary_size * eliminated_size);
        for (Index eliminated = 0; eliminated < eliminated_count; ++eliminated)
        {
            const auto& entries = adjacency[static_cast<std::size_t>(eliminated)];
            transformed.resize(entries.size() * transformed_block_size);
            const Scalar* inverse =
                eliminated_inverse.data() + static_cast<std::size_t>(eliminated * eliminated_size * eliminated_size);
            for (std::size_t left = 0; left < entries.size(); ++left)
            {
                const auto& cross = cross_blocks[entries[left]];
                Scalar* transformed_cross = transformed.data() + left * transformed_block_size;
                if (primary_size == 9 && eliminated_size == 3)
                {
                    transform9x3(inverse, cross.values.data(), transformed_cross);
                }
                else
                {
                    for (Index row = 0; row < primary_size; ++row)
                    {
                        multiplyMatrixVector(inverse,
                                             ...
                                             eliminated_size,
                                             cross.values.data() + row * eliminated_size,
                                             transformed_cross + row * eliminated_size);
                    }
                }
            }
            const auto& pair_slots = SchurComplementSolverWorkspaceAccess::eliminatedPairSlots(workspace, eliminated);
            for (std::size_t left = 0; left < entries.size(); ++left)
            {
                const Scalar* transformed_cross = transformed.data() + left * transformed_block_size;
                for (std::size_t right = 0; right < entries.size(); ++right)
                {
                    ...
                    for (Index row = 0; row < primary_size; ++row)
                    {
                        for (Index column = 0; column < primary_size; ++column)
                        {
                            const Scalar* left_row = transformed_cross + row * eliminated_size;
                            const Scalar* right_row = right_cross.values.data() + column * eliminated_size;
                            Scalar value = eliminated_size == 3 ? dot3(left_row, right_row) : Scalar(0);
                            if (eliminated_size != 3)
                            {
                                for (Index inner = 0; inner < eliminated_size; ++inner)
                                {
                                    value += left_row[inner] * right_row[inner];
                                }
                            }

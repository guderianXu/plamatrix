...
namespace plamatrix
{
    ...
    BlockNormalEquations<Scalar>::BlockNormalEquations(Index primary_block_count,
                                                       Index eliminated_block_count,
                                                       Index primary_block_size,
                                                       Index eliminated_block_size)
        : _primaryBlockCount(primary_block_count), _eliminatedBlockCount(eliminated_block_count),
          _primaryBlockSize(primary_block_size), _eliminatedBlockSize(eliminated_block_size)
    {
        static_assert(std::is_floating_point_v<Scalar>, "BlockNormalEquations requires a floating-point scalar");
        if (primary_block_count < 0 || eliminated_block_count < 0 || primary_block_size <= 0 ||
    ...
    template <typename Scalar>
    void BlockNormalEquations<Scalar>::validateResidual(const Scalar* jacobian,
                                                        Index jacobian_size,
                                                        ...
                                                        const char* operation) const
    {
        ...
        {
            throw std::invalid_argument(std::string(operation) + ": weight must be finite and non-negative");
        }
        for (Index index = 0; index < jacobian_size; ++index)
        {
            if (!std::isfinite(jacobian[index]))
            {
                throw std::invalid_argument(std::string(operation) + ": Jacobian values must be finite");
            }
        }
        for (Index index = 0; index < residual_size; ++index)
        {
            ...
            {
                throw std::invalid_argument(std::string(operation) + ": residual values must be finite");
            }
        }
    }
    ...
    template <typename Scalar>
    std::size_t BlockNormalEquations<Scalar>::findOrCreatePrimaryCrossBlock(Index row_block, Index column_block)
    {
    ...
    template <typename Scalar>
    void BlockNormalEquations<Scalar>::addPrimaryTerms(const Index* primary_blocks,
                                                       ...
                                                       Scalar weight)
    {
        if (!primary_blocks || !primary_jacobians || primary_block_count == 0)
        {
            throw std::invalid_argument(
                "BlockNormalEquations: primary block and Jacobian lists must have equal non-zero size");
        }
        for (std::size_t index = 0; index < primary_block_count; ++index)
        {
            ...
            if (std::find(primary_blocks, primary_blocks + index, block) != primary_blocks + index)
            {
        ...
        for (std::size_t block_index = 0; block_index < primary_block_count; ++block_index)
        {
            ...
            for (Index row = 0; row < residual_size; ++row)
            {
                for (Index column = 0; column < _primaryBlockSize; ++column)
                {
                    ...
                    {
                        _primaryDiagonal[static_cast<std::size_t>(matrix_offset + column * _primaryBlockSize +
                                                                  other)] +=
        ...
        for (std::size_t left = 0; left < primary_block_count; ++left)
        {
            for (std::size_t right = left + 1; right < primary_block_count; ++right)
            {
                ...
                const Scalar* row_jacobian = ordered ? primary_jacobians[left] : primary_jacobians[right];
                const Scalar* column_jacobian = ordered ? primary_jacobians[right] : primary_jacobians[left];
                auto& cross = _primaryCrossBlocks[findOrCreatePrimaryCrossBlock(row_block, column_block)].values;
                for (Index residual_row = 0; residual_row < residual_size; ++residual_row)
                {
                    for (Index row = 0; row < _primaryBlockSize; ++row)
                    {
                        const Scalar row_value = row_jacobian[residual_row * _primaryBlockSize + row];
                        for (Index column = 0; column < _primaryBlockSize; ++column)
                        {
                            cross[static_cast<std::size_t>(row * _primaryBlockSize + column)] +=
                                weight * row_value * column_jacobian[residual_row * _primaryBlockSize + column];
                        }
                    }
                }
            }
    ...
    template <typename Scalar>
    void BlockNormalEquations<Scalar>::addResidualBlock(Index primary_block,
                                                        ...
                                                        Scalar weight)
    {
        const Index primary_blocks[] = {primary_block};
        const Scalar* primary_jacobians[] = {primary_jacobian};
        addResidualBlocks(primary_blocks,
                          primary_jacobians,
                          1,
                          eliminated_block,
                          eliminated_jacobian,
    ...
    template <typename Scalar>
    void BlockNormalEquations<Scalar>::addResidualBlocks(const std::vector<Index>& primary_blocks,
                                                         ...
                                                         Scalar weight)
    {
        if (primary_blocks.size() != primary_jacobians.size())
        {
            throw std::invalid_argument("BlockNormalEquations: primary block and Jacobian lists must have equal size");
        }
        addResidualBlocks(primary_blocks.data(),
    ...
    template <typename Scalar>
    void BlockNormalEquations<Scalar>::addResidualBlocks(const Index* primary_blocks,
                                                         const Scalar* const* primary_jacobians,
                                                         std::size_t primary_block_count,
                                                         Index eliminated_block,
                                                         const Scalar* eliminated_jacobian,
                                                         ...
                                                         Scalar weight)
    {
        ...
        addPrimaryTerms(primary_blocks, primary_jacobians, primary_block_count, residual, residual_size, weight);

        const Index eliminated_matrix_offset = eliminated_block * _eliminatedBlockSize * _eliminatedBlockSize;
        const Index eliminated_gradient_offset = eliminated_block * _eliminatedBlockSize;
        for (std::size_t primary_index = 0; primary_index < primary_block_count; ++primary_index)
        {
            auto& cross = _crossBlocks[findOrCreateCrossBlock(primary_blocks[primary_index], eliminated_block)].values;
            const Scalar* primary_jacobian = primary_jacobians[primary_index];
            for (Index row = 0; row < residual_size; ++row)
            {
                for (Index primary_column = 0; primary_column < _primaryBlockSize; ++primary_column)
                {
                    const Scalar primary_value = primary_jacobian[row * _primaryBlockSize + primary_column];
                    for (Index eliminated_column = 0; eliminated_column < _eliminatedBlockSize; ++eliminated_column)
                    {
                        cross[static_cast<std::size_t>(primary_column * _eliminatedBlockSize + eliminated_column)] +=
        ...
        for (Index row = 0; row < residual_size; ++row)
        {
            ...
            {
                const Scalar eliminated_value = eliminated_jacobian[row * _eliminatedBlockSize + eliminated_col];
                _eliminatedGradient[static_cast<std::size_t>(eliminated_gradient_offset + eliminated_col)] +=
                    weight * eliminated_value * residual[row];
                for (Index other = 0; other < _eliminatedBlockSize; ++other)
                {
                    _eliminatedDiagonal[static_cast<std::size_t>(eliminated_matrix_offset +
                                                                 eliminated_col * _eliminatedBlockSize + other)] +=
                        weight * eliminated_value * eliminated_jacobian[row * _eliminatedBlockSize + other];
                }
            }
        }
    }
    ...
    void BlockNormalEquations<Scalar>::addPrimaryResidualBlock(
        Index primary_block, const Scalar* primary_jacobian, const Scalar* residual, Index residual_size, Scalar weight)
    {
        const Index primary_blocks[] = {primary_block};
        const Scalar* primary_jacobians[] = {primary_jacobian};
        addPrimaryResidualBlocks(primary_blocks, primary_jacobians, 1, residual, residual_size, weight);
    }

    template <typename Scalar>
    void BlockNormalEquations<Scalar>::addPrimaryResidualBlocks(const std::vector<Index>& primary_blocks,
                                                                const std::vector<const Scalar*>& primary_jacobians,
                                                                const Scalar* residual,
                                                                Index residual_size,
                                                                Scalar weight)
    {
        ...
        addPrimaryResidualBlocks(
            primary_blocks.data(), primary_jacobians.data(), primary_blocks.size(), residual, residual_size, weight);
    }

    template <typename Scalar>
    void BlockNormalEquations<Scalar>::addPrimaryResidualBlocks(const Index* primary_blocks,
                                                                const Scalar* const* primary_jacobians,
                                                                std::size_t primary_block_count,
                                                                const Scalar* residual,
                                                                Index residual_size,
    ...
    template <typename Scalar>
    void BlockNormalEquations<Scalar>::addEliminatedResidualBlock(Index eliminated_block,
                                                                  const Scalar* eliminated_jacobian,
                                                                  const Scalar* residual,
                                                                  Index residual_size,
                                                                  Scalar weight)
    {
        if (eliminated_block < 0 || eliminated_block >= _eliminatedBlockCount)
        {
            throw std::out_of_range("BlockNormalEquations::addEliminatedResidualBlock: block index out of range");
        }
        validateResidual(eliminated_jacobian,
                         residual_size * _eliminatedBlockSize,
                         residual,
        ...
        for (Index row = 0; row < residual_size; ++row)
        {
            for (Index col = 0; col < _eliminatedBlockSize; ++col)
            {
                ...
                _eliminatedGradient[static_cast<std::size_t>(gradient_offset + col)] += weight * value * residual[row];
                for (Index other = 0; other < _eliminatedBlockSize; ++other)
                {
                    _eliminatedDiagonal[static_cast<std::size_t>(matrix_offset + col * _eliminatedBlockSize + other)] +=
    ...
    template <typename Scalar> void BlockNormalEquations<Scalar>::mergeFrom(const BlockNormalEquations& other)
    {
        if (_primaryBlockCount != other._primaryBlockCount || _eliminatedBlockCount != other._eliminatedBlockCount ||
            _primaryBlockSize != other._primaryBlockSize || _eliminatedBlockSize != other._eliminatedBlockSize)
        {
            throw std::invalid_argument("BlockNormalEquations::mergeFrom: block layouts must match");
        }
        const auto add_values = [](std::vector<Scalar>* target, const std::vector<Scalar>& source)
        {
    ...
    template <typename Scalar> void BlockNormalEquations<Scalar>::clearValues() noexcept
    {
        std::fill(_primaryDiagonal.begin(), _primaryDiagonal.end(), Scalar(0));
        std::fill(_eliminatedDiagonal.begin(), _eliminatedDiagonal.end(), Scalar(0));
        std::fill(_primaryGradient.begin(), _primaryGradient.end(), Scalar(0));
        std::fill(_eliminatedGradient.begin(), _eliminatedGradient.end(), Scalar(0));
        for (auto& cross : _primaryCrossBlocks)
        {
            std::fill(cross.values.begin(), cross.values.end(), Scalar(0));
        }
        for (auto& cross : _crossBlocks)
        {
            std::fill(cross.values.begin(), cross.values.end(), Scalar(0));
        }
    }

    template <typename Scalar> Index BlockNormalEquations<Scalar>::primaryBlockCount() const noexcept
    {
        return _primaryBlockCount;
    }

    template <typename Scalar> Index BlockNormalEquations<Scalar>::eliminatedBlockCount() const noexcept
    {
        return _eliminatedBlockCount;
    }

    template <typename Scalar> Index BlockNormalEquations<Scalar>::primaryBlockSize() const noexcept
    {
        return _primaryBlockSize;
    }

    template <typename Scalar> Index BlockNormalEquations<Scalar>::eliminatedBlockSize() const noexcept
    {

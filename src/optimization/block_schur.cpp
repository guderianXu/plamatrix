...
namespace plamatrix
{
                                                                   ...
                                                                   std::vector<Scalar>* eliminated_step)
    {
        ...
        return solveDampedSchurComplement(equations, damping, options, workspace, primary_step, eliminated_step);
    }
    ...
    SchurComplementSolverReport<Scalar> solveDampedSchurComplement(const BlockNormalEquations<Scalar>& equations,
                                                                   ...
                                                                   std::vector<Scalar>* eliminated_step)
    {
        static_assert(std::is_floating_point_v<Scalar>, "solveDampedSchurComplement requires a floating-point scalar");
        if (!primary_step || !eliminated_step || primary_step == eliminated_step)
        {
            throw std::invalid_argument("solveDampedSchurComplement: output vectors must be non-null and distinct");
        }
        ...
        if (!options.useInitialGuess || primary_step->size() != static_cast<std::size_t>(primary_dimension))
        {
        ...
        auto& reduced_rhs = block_schur_detail::SchurComplementSolverWorkspaceAccess::hostReducedRhs(workspace);
        auto& host_scratch = block_schur_detail::SchurComplementSolverWorkspaceAccess::hostScratch(workspace);
        primary_diagonal.assign(equations._primaryDiagonal.begin(), equations._primaryDiagonal.end());
        eliminated_diagonal.assign(equations._eliminatedDiagonal.begin(), equations._eliminatedDiagonal.end());
        const auto apply_damping = [&](std::vector<Scalar>& blocks, Index count, Index size)
        {
            for (Index block = 0; block < count; ++block)
            {
                ...
                {
                    Scalar& value = blocks[static_cast<std::size_t>(offset + diagonal * size + diagonal)];
        ...
        eliminated_inverse.resize(static_cast<std::size_t>(eliminated_count * eliminated_size * eliminated_size));
        host_scratch.resize(static_cast<std::size_t>(eliminated_size * eliminated_size + 3 * eliminated_size));
        ...
        for (Index block = 0; block < eliminated_count; ++block)
        {
            ...
            Scalar* inverse =
                eliminated_inverse.data() + static_cast<std::size_t>(block * eliminated_size * eliminated_size);
            const bool inverted = eliminated_size == 3
                                      ...
                                      : block_schur_detail::invertPositiveDefiniteInto(
                                            matrix, eliminated_size, inverse, inverse_lower, inverse_intermediate);
            if (!inverted)
            {
        ...
        report.smallBlockInverseSeconds =
            std::chrono::duration<double>(std::chrono::steady_clock::now() - inverse_start).count();

        reduced_rhs.assign(static_cast<std::size_t>(primary_dimension), Scalar(0));
        for (Index index = 0; index < primary_dimension; ++index)
        {
            reduced_rhs[static_cast<std::size_t>(index)] = -equations._primaryGradient[static_cast<std::size_t>(index)];
        }
        Scalar* eliminated_product = host_scratch.data();
        for (Index block = 0; block < eliminated_count; ++block)
        {
            multiplyMatrixVector(eliminated_inverse.data() +
                                     static_cast<std::size_t>(block * eliminated_size * eliminated_size),
                                 eliminated_size,
                                 eliminated_size,
                                 equations._eliminatedGradient.data() + block * eliminated_size,
                                 eliminated_product);
            for (const std::size_t cross_index : equations._eliminatedAdjacency[static_cast<std::size_t>(block)])
            {
        ...
        if (options.linearBackend != SchurComplementLinearBackend::DenseCpu)
        {
            ...
            {
                primary_adjacency[static_cast<std::size_t>(equations._crossBlocks[index].primaryBlock)].push_back(
                    index);
            }
            ...
            for (Index block = 0; block < primary_count; ++block)
            {
                ...
                for (const std::size_t cross_index : primary_adjacency[static_cast<std::size_t>(block)])
                {
                    const auto& cross = equations._crossBlocks[cross_index];
                    const Scalar* inverse =
                        eliminated_inverse.data() +
                        static_cast<std::size_t>(cross.eliminatedBlock * eliminated_size * eliminated_size);
                    if (primary_size == 9 && eliminated_size == 3)
                    {
                        block_schur_detail::transform9x3(inverse, cross.values.data(), temporary_matrix.data());
                    }
                    else
                    ...
                    for (Index row = 0; row < primary_size; ++row)
                    {
                        for (Index col = 0; col < primary_size; ++col)
                        {
                            const Scalar* transformed =
                                temporary_matrix.data() + static_cast<std::size_t>(row * eliminated_size);
                            const Scalar* source_row =
                                cross.values.data() + static_cast<std::size_t>(col * eliminated_size);
                            Scalar value =
                                eliminated_size == 3 ? block_schur_detail::dot3(transformed, source_row) : Scalar(0);
                            for (Index inner = 0; inner < (eliminated_size == 3 ? 0 : eliminated_size); ++inner)
                            {
                ...
                if (!invertPositiveDefinite(
                        schur_diagonal.data(), primary_size, &preconditioner_inverse[static_cast<std::size_t>(block)]))
                {
            ...
            report.smallBlockInverseSeconds +=
                std::chrono::duration<double>(std::chrono::steady_clock::now() - preconditioner_inverse_start).count();

            if (options.linearBackend == SchurComplementLinearBackend::Cpu && options.preconditionerClusterSize > 1 &&
                primary_count > 1)
            {
                const auto cluster_start = std::chrono::steady_clock::now();
                preconditioner_cluster_size =
                    std::min<Index>(primary_count, static_cast<Index>(options.preconditionerClusterSize));
                const Index cluster_count =
                    (primary_count + preconditioner_cluster_size - 1) / preconditioner_cluster_size;
                std::vector<std::vector<Scalar>> cluster_matrices(static_cast<std::size_t>(cluster_count));
                for (Index cluster = 0; cluster < cluster_count; ++cluster)
                {
                    const Index block_begin = cluster * preconditioner_cluster_size;
                    const Index block_count = std::min(preconditioner_cluster_size, primary_count - block_begin);
                    const Index dimension = block_count * primary_size;
                    auto& matrix = cluster_matrices[static_cast<std::size_t>(cluster)];
                    matrix.assign(static_cast<std::size_t>(dimension * dimension), Scalar(0));
                    for (Index block_offset = 0; block_offset < block_count; ++block_offset)
                    {
                        const Scalar* diagonal =
                            primary_diagonal.data() + (block_begin + block_offset) * primary_size * primary_size;
                        for (Index row = 0; row < primary_size; ++row)
                        {
                            for (Index column = 0; column < primary_size; ++column)
                            {
                                matrix[static_cast<std::size_t>((block_offset * primary_size + row) * dimension +
                                                                block_offset * primary_size + column)] =
                                    diagonal[row * primary_size + column];
                            }
                        }
                    }
                }
                ...
                {
                    const Index row_cluster = cross.rowBlock / preconditioner_cluster_size;
                    const Index column_cluster = cross.columnBlock / preconditioner_cluster_size;
                    if (row_cluster != column_cluster)
                    {
                        continue;
                    }
                    const Index block_begin = row_cluster * preconditioner_cluster_size;
                    const Index block_count = std::min(preconditioner_cluster_size, primary_count - block_begin);
                    const Index dimension = block_count * primary_size;
                    const Index local_row = cross.rowBlock - block_begin;
                    const Index local_column = cross.columnBlock - block_begin;
                    auto& matrix = cluster_matrices[static_cast<std::size_t>(row_cluster)];
                    for (Index row = 0; row < primary_size; ++row)
                    {
                        for (Index column = 0; column < primary_size; ++column)
                        {
                            const Scalar value = cross.values[static_cast<std::size_t>(row * primary_size + column)];
                            matrix[static_cast<std::size_t>((local_row * primary_size + row) * dimension +
                                                            local_column * primary_size + column)] += value;
                            matrix[static_cast<std::size_t>((local_column * primary_size + column) * dimension +
                                                            local_row * primary_size + row)] += value;
                        }
                    }
                }

                std::vector<Scalar> transformed(static_cast<std::size_t>(primary_size * eliminated_size));
                for (Index eliminated_block = 0; eliminated_block < eliminated_count; ++eliminated_block)
                {
                    const auto& adjacency = equations._eliminatedAdjacency[static_cast<std::size_t>(eliminated_block)];
                    const Scalar* inverse =
                        eliminated_inverse.data() + eliminated_block * eliminated_size * eliminated_size;
                    for (std::size_t left_index = 0; left_index < adjacency.size(); ++left_index)
                    {
                        const auto& left = equations._crossBlocks[adjacency[left_index]];
                        const Index cluster = left.primaryBlock / preconditioner_cluster_size;
                        if (primary_size == 9 && eliminated_size == 3)
                        {
                            block_schur_detail::transform9x3(inverse, left.values.data(), transformed.data());
                        }
                        else
                            for (Index row = 0; row < primary_size; ++row)
                            {
                                multiplyMatrixVector(inverse,
                                                     eliminated_size,
                                                     eliminated_size,
                                                     left.values.data() + row * eliminated_size,
                                                     transformed.data() + row * eliminated_size);
                            }
                        for (std::size_t right_index = left_index + 1; right_index < adjacency.size(); ++right_index)
                        {
                            const auto& right = equations._crossBlocks[adjacency[right_index]];
                            if (right.primaryBlock / preconditioner_cluster_size != cluster)
                            {
                                continue;
                            }
                            const Index block_begin = cluster * preconditioner_cluster_size;
                            const Index block_count =
                                std::min(preconditioner_cluster_size, primary_count - block_begin);
                            const Index dimension = block_count * primary_size;
                            const Index local_left = left.primaryBlock - block_begin;
                            const Index local_right = right.primaryBlock - block_begin;
                            auto& matrix = cluster_matrices[static_cast<std::size_t>(cluster)];
                            for (Index row = 0; row < primary_size; ++row)
                            {
                                for (Index column = 0; column < primary_size; ++column)
                                {
                                    const Scalar* transformed_row = transformed.data() + row * eliminated_size;
                                    const Scalar* right_row = right.values.data() + column * eliminated_size;
                                    Scalar value = Scalar(0);
                                    for (Index inner = 0; inner < eliminated_size; ++inner)
                                    {
                                        value += transformed_row[inner] * right_row[inner];
                                    }
                                    matrix[static_cast<std::size_t>((local_left * primary_size + row) * dimension +
                                                                    local_right * primary_size + column)] -= value;
                                    matrix[static_cast<std::size_t>((local_right * primary_size + column) * dimension +
                                                                    local_left * primary_size + row)] -= value;
                                }
                            }
                        }
                    }
                ...
                cluster_preconditioner_inverse.reserve(static_cast<std::size_t>(cluster_count));
                for (Index cluster = 0; cluster < cluster_count; ++cluster)
                {
                    const Index block_begin = cluster * preconditioner_cluster_size;
                    const Index block_count = std::min(preconditioner_cluster_size, primary_count - block_begin);
                    const Index dimension = block_count * primary_size;
                    std::vector<Scalar> inverse;
                    if (!invertPositiveDefinite(
                            cluster_matrices[static_cast<std::size_t>(cluster)].data(), dimension, &inverse))
                    {
                        cluster_preconditioner_inverse.clear();
                        break;
                    }
                    cluster_preconditioner_inverse.push_back(std::move(inverse));
                }
                report.smallBlockInverseSeconds +=
        ...
        const auto apply_preconditioner = [&](const std::vector<Scalar>& input, std::vector<Scalar>* output)
        {
            if (!cluster_preconditioner_inverse.empty())
            {
                for (Index cluster = 0; cluster < static_cast<Index>(cluster_preconditioner_inverse.size()); ++cluster)
                {
                    const Index block_begin = cluster * preconditioner_cluster_size;
                    const Index block_count = std::min(preconditioner_cluster_size, primary_count - block_begin);
                    const Index dimension = block_count * primary_size;
                    multiplyMatrixVector(cluster_preconditioner_inverse[static_cast<std::size_t>(cluster)].data(),
                                         dimension,
                                         dimension,
                                         input.data() + block_begin * primary_size,
                                         output->data() + block_begin * primary_size);
                }
                return;
            }
            for (Index block = 0; block < primary_count; ++block)
        ...
        report.preconditionerName = cluster_preconditioner_inverse.empty()
                                        ? "block_jacobi"
                                        : "cluster_jacobi_" + std::to_string(preconditioner_cluster_size);
        ...
        else if (options.linearBackend == SchurComplementLinearBackend::DenseCpu)
        {
            ...
            block_schur_detail::assembleReducedSchurDenseLower(primary_count,
                                                               eliminated_count,
                                                               primary_size,
                                                               eliminated_size,
                                                               primary_diagonal,
        ...
        else if (options.linearBackend != SchurComplementLinearBackend::Cpu)
        {
            ...
            auto accelerated_report = block_schur_detail::solveAcceleratedReducedSchur(
                schur_matrix, reduced_rhs, preconditioner_inverse, primary_size, options, workspace, primary_step);
            ...
            accelerated_report.preconditionerName = "block_jacobi";
        ...
        for (Index block = 0; block < eliminated_count; ++block)
        {
            ...
            {
                const auto& cross = equations._crossBlocks[cross_index];
                const Scalar* primary = primary_step->data() + cross.primaryBlock * primary_size;
                for (Index column = 0; column < eliminated_size; ++column)
                {
                    Scalar value = Scalar(0);
                    for (Index row = 0; row < primary_size; ++row)
                    {
                        value += cross.values[static_cast<std::size_t>(row * eliminated_size + column)] * primary[row];
                    }
        ...
        report.backSubstitutionSeconds =
            std::chrono::duration<double>(std::chrono::steady_clock::now() - back_substitution_start).count();
        report.message = "converged";
        return report;
    }
    template SchurComplementSolverReport<float> solveDampedSchurComplement(const BlockNormalEquations<float>&,
                                                                           float,
                                                                           const SchurComplementSolverOptions<float>&,
                                                                           std::vector<float>*,
                                                                           std::vector<float>*);
    template SchurComplementSolverReport<double> solveDampedSchurComplement(const BlockNormalEquations<double>&,
                                                                            double,

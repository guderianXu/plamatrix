...
namespace plamatrix
{
    namespace opencl
    {
        namespace
        {
            ...
            std::size_t localSize(OpenClRuntime& runtime)
            {
                ...
                checkOpenCl(clGetDeviceInfo(
                                runtime.device(), CL_DEVICE_MAX_WORK_GROUP_SIZE, sizeof(maximum), &maximum, nullptr),
            ...
            void launch(cl_command_queue queue,
                        cl_kernel kernel,
                        std::size_t size,
            ...
            double dot(cl_command_queue queue,
                       CompiledKernel& dot_kernel,
                       CompiledKernel& reduce_kernel,
                       const DeviceBuffer& first,
                       const DeviceBuffer& second,
                       DeviceBuffer& partial_a,
                       DeviceBuffer& partial_b,
                       Index size,
                       std::size_t local_size,
                       bool double_accumulation)
            {
                ...
                if (double_accumulation)
                {
                    ...
                    checkOpenCl(clEnqueueReadBuffer(
                                    queue, current->get(), CL_TRUE, 0, sizeof(result), &result, 0, nullptr, nullptr),
                                             ...
                                             Index block_size)
            {
                ...
                {
                    throw std::invalid_argument("OpenCL block PCG requires complete row-major inverse diagonal blocks");
                }
                if (inverse_blocks.data() == rhs.data() || inverse_blocks.data() == solution.data())
                {
                    throw std::invalid_argument("OpenCL block PCG inverse blocks must not alias rhs or solution");
                }
                for (Index index = 0; index < inverse_blocks.rows(); ++index)
                {
                    if (!std::isfinite(inverse_blocks.data()[index]))
                    {
                        throw std::invalid_argument("OpenCL block PCG inverse blocks must contain finite values");
                    }
                }
            }
                                        ...
                                        Index block_size)
            {
                ...
                {
                    validateBlockPreconditioner(matrix, rhs, solution, *inverse_blocks, block_size);
                }
                ...
                DeviceBuffer rows(runtime.context(),
                                  CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                                  byteSize<Index>(count + 1),
                                  const_cast<Index*>(matrix.rowOffsets()));
                DeviceBuffer columns(runtime.context(),
                                     CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                                     byteSize<Index>(static_cast<std::size_t>(matrix.nnz())),
                                     const_cast<Index*>(matrix.colIndices()));
                DeviceBuffer values(runtime.context(),
                                    CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                                    byteSize<Scalar>(static_cast<std::size_t>(matrix.nnz())),
                                    const_cast<Scalar*>(matrix.values()));
                DeviceBuffer rhs_buffer(runtime.context(),
                                        CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                                        byteSize<Scalar>(count),
                                        const_cast<Scalar*>(rhs.data()));
                DeviceBuffer x(runtime.context(),
                               CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
                               byteSize<Scalar>(count),
                               solution.data());
                const Scalar* inverse_data =
                    inverse_blocks != nullptr ? inverse_blocks->data() : inverse_diagonal.data();
                const std::size_t inverse_count =
                    inverse_blocks != nullptr ? static_cast<std::size_t>(inverse_blocks->rows()) : count;
                DeviceBuffer inverse(runtime.context(),
                                     CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                                     byteSize<Scalar>(inverse_count),
                                     const_cast<Scalar*>(inverse_data));
                ...
                const std::size_t partial_bytes =
                    double_accumulation ? byteSize<double>(partial_count) : byteSize<Scalar>(partial_count);
                ...
                {
                    kernelBufferArg(spmv_kernel, 0, rows);
                    kernelBufferArg(spmv_kernel, 1, columns);
                    kernelBufferArg(spmv_kernel, 2, values);
                    kernelBufferArg(spmv_kernel, 3, input);
                    kernelBufferArg(spmv_kernel, 4, output);
                    kernelArg(spmv_kernel, 5, device_count);
                ...
                if (inverse_blocks != nullptr)
                {
                    ...
                    launch(queue.get(), copy_kernel, count, local_size, "clEnqueueNDRangeKernel(copy direction)");
                }
                else
                {
                ...
                double residual_squared = dot<Scalar>(queue.get(),
                                                      dot_kernel,
                                                      reduce_kernel,
                                                      residual,
                                                      residual,
                ...
                double rho = dot<Scalar>(queue.get(),
                                         dot_kernel,
                                         reduce_kernel,
                                         residual,
                                         transformed,
                ...
                for (int iteration = 0; !report.converged && iteration < options.maxIterations; ++iteration)
                {
                    ...
                    const double denominator = dot<Scalar>(queue.get(),
                                                           dot_kernel,
                                                           reduce_kernel,
                                                           direction,
                                                           matrix_direction,
                    ...
                    kernelBufferArg(update_kernel, 0, x);
                    kernelBufferArg(update_kernel, 1, residual);
                    kernelBufferArg(update_kernel, 2, direction);
                    kernelBufferArg(update_kernel, 3, matrix_direction);
                    ...
                    if (report.converged)
                        break;
                    run_preconditioner();
                    const double next_rho = dot<Scalar>(queue.get(),
                                                        dot_kernel,
                                                        reduce_kernel,
                                                        residual,
                                                        transformed,
                    ...
                    kernelBufferArg(direction_kernel, 0, direction);
                    kernelBufferArg(direction_kernel, 1, transformed);
                    kernelArg(direction_kernel, 2, beta);
                    kernelArg(direction_kernel, 3, device_count);
                    launch(queue.get(), direction_kernel, count, local_size, "clEnqueueNDRangeKernel(direction)");
                    rho = next_rho;
                }
                checkOpenCl(clEnqueueReadBuffer(queue.get(),
                                                x.get(),
                                                CL_TRUE,
                                                0,
                                                byteSize<Scalar>(count),
        ...
        IterativeSolverReport blockPcg(const CSRMatrix<Scalar, Device::CPU>& matrix,
                                       ...
                                       const IterativeSolverOptions& options)
        {
            return solve(matrix, rhs, solution, options, &inverse_blocks, block_size);
        }

...
struct IndexedReductionResult
{
    ...
    DenseStorage<Index, Dev> indices;
};
...
DenseStorage<Scalar, Device::CPU> sum(
    const DenseStorage<Scalar, Device::CPU>& input,
    ReductionAxis axis);

/// Return the column-major coefficient-wise dot product without allocating an intermediate matrix.
/// Float inputs accumulate in double. Empty inputs return zero.
...
/// Return the dot product of arbitrary-positive-stride CPU views.
template <typename Scalar>
Scalar dot(
    ConstMatrixView<Scalar, Device::CPU> lhs,
    ConstMatrixView<Scalar, Device::CPU> rhs);

/// Return dot(input, input) without allocating an intermediate matrix.
/// Float inputs accumulate in double. Empty inputs return zero.
template <typename Scalar>
Scalar squaredNorm(const DenseStorage<Scalar, Device::CPU>& input);

/// Return the squared norm of an arbitrary-positive-stride CPU view.
template <typename Scalar>
Scalar squaredNorm(ConstMatrixView<Scalar, Device::CPU> input);
...
template <typename Scalar>
DenseStorage<Scalar, Device::CPU> mean(
    const DenseStorage<Scalar, Device::CPU>& input,
    ReductionAxis axis);
...
/// @throws std::invalid_argument if a non-empty output contains an empty lane or axis is invalid
template <typename Scalar>
DenseStorage<Scalar, Device::CPU> min(
    const DenseStorage<Scalar, Device::CPU>& input,
    ReductionAxis axis);
...
/// @throws std::invalid_argument if a non-empty output contains an empty lane or axis is invalid
template <typename Scalar>
DenseStorage<Scalar, Device::CPU> max(
    const DenseStorage<Scalar, Device::CPU>& input,
    ReductionAxis axis);
...
template <typename Scalar>
DenseStorage<Scalar, Device::GPU> sum(
...
template <typename Scalar>
DenseStorage<Scalar, Device::GPU> sumAsync(
    const DenseStorage<Scalar, Device::GPU>& input,
    ReductionAxis axis,
    ReductionWorkspace& workspace,
    cudaStream_t stream);
...
template <typename Scalar>
DenseStorage<Scalar, Device::GPU> mean(
    const DenseStorage<Scalar, Device::GPU>& input,
    ReductionAxis axis);
...
template <typename Scalar>
DenseStorage<Scalar, Device::GPU> meanAsync(
    const DenseStorage<Scalar, Device::GPU>& input,
    ReductionAxis axis,
    ReductionWorkspace& workspace,
    cudaStream_t stream);
...
template <typename Scalar>
DenseStorage<Scalar, Device::GPU> min(
    const DenseStorage<Scalar, Device::GPU>& input,
    ReductionAxis axis);
...
template <typename Scalar>
DenseStorage<Scalar, Device::GPU> minAsync(
    const DenseStorage<Scalar, Device::GPU>& input,
    ReductionAxis axis,
    ReductionWorkspace& workspace,
    cudaStream_t stream);
...
template <typename Scalar>
DenseStorage<Scalar, Device::GPU> max(
    const DenseStorage<Scalar, Device::GPU>& input,
    ReductionAxis axis);
...
template <typename Scalar>
DenseStorage<Scalar, Device::GPU> maxAsync(
    const DenseStorage<Scalar, Device::GPU>& input,
    ReductionAxis axis,
    ReductionWorkspace& workspace,
    cudaStream_t stream);
...
/// Reduce caller-owned contiguous column-major GPU views without allocating result matrices.
/// Output shape follows ReductionAxis; input and output storage must not overlap.
template <typename Scalar>
void sum(ConstMatrixView<Scalar, Device::GPU> input,
         ReductionAxis axis,
...
template <typename Scalar>
void sumAsync(ConstMatrixView<Scalar, Device::GPU> input,
              ReductionAxis axis,
              MatrixView<Scalar, Device::GPU> output,
              ReductionWorkspace& workspace,
              cudaStream_t stream);
...
template <typename Scalar>
void min(ConstMatrixView<Scalar, Device::GPU> input,
         ReductionAxis axis,
         MatrixView<Scalar, Device::GPU> output,
         ReductionWorkspace& workspace,
         cudaStream_t stream = nullptr);
...
template <typename Scalar>
void minAsync(ConstMatrixView<Scalar, Device::GPU> input,
              ReductionAxis axis,
              MatrixView<Scalar, Device::GPU> output,
              ReductionWorkspace& workspace,
              cudaStream_t stream);
...
template <typename Scalar>
void max(ConstMatrixView<Scalar, Device::GPU> input,
         ReductionAxis axis,
         MatrixView<Scalar, Device::GPU> output,
         ReductionWorkspace& workspace,
         cudaStream_t stream = nullptr);
...
template <typename Scalar>
void maxAsync(ConstMatrixView<Scalar, Device::GPU> input,
              ReductionAxis axis,
              MatrixView<Scalar, Device::GPU> output,
              ReductionWorkspace& workspace,
              cudaStream_t stream);
...
#define PLAMATRIX_NO_CUDA_VALUE_REDUCTION(OP, ASYNC_OP)                                  \
    ...
    template <typename Scalar>                                                           \
    void OP(const DenseStorage<Scalar, Device::GPU>&, ReductionAxis,                      \
    ...
    template <typename Scalar>                                                           \
    void ASYNC_OP(const DenseStorage<Scalar, Device::GPU>&, ReductionAxis,                \
                  DenseStorage<Scalar, Device::GPU>&, ReductionWorkspace&, cudaStream_t)  \
    {                                                                                    \
        detail::throwReductionNoCuda(#ASYNC_OP " requires PLAMATRIX_WITH_CUDA=ON");      \
    }
...
#define PLAMATRIX_NO_CUDA_VIEW_VALUE_REDUCTION(OP, ASYNC_OP)                      \
    template <typename Scalar>                                                    \
    inline void OP(ConstMatrixView<Scalar, Device::GPU>, ReductionAxis,            \
                   MatrixView<Scalar, Device::GPU>, ReductionWorkspace&,          \
                   cudaStream_t)                                                  \
    {                                                                              \
        detail::throwReductionNoCuda(#OP " requires PLAMATRIX_WITH_CUDA=ON");     \
    }                                                                              \
    template <typename Scalar>                                                    \
    inline void ASYNC_OP(ConstMatrixView<Scalar, Device::GPU>, ReductionAxis,      \
                         MatrixView<Scalar, Device::GPU>, ReductionWorkspace&,    \
                         cudaStream_t)                                            \
    {                                                                              \
        detail::throwReductionNoCuda(#ASYNC_OP " requires PLAMATRIX_WITH_CUDA=ON"); \
...
PLAMATRIX_NO_CUDA_VIEW_VALUE_REDUCTION(sum, sumAsync)
PLAMATRIX_NO_CUDA_VIEW_VALUE_REDUCTION(min, minAsync)
PLAMATRIX_NO_CUDA_VIEW_VALUE_REDUCTION(max, maxAsync)

#undef PLAMATRIX_NO_CUDA_VIEW_VALUE_REDUCTION

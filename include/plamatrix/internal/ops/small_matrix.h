...
{
    DenseStorage<Scalar, Dev> eigenvalues;
    DenseStorage<Scalar, Dev> eigenvectors;
};
...
/// Decompose caller-owned contiguous column-major GPU views without allocating output matrices.
/// Input is N x 6; eigenvalues and eigenvectors are N x 3 and N x 9. All storage must be disjoint.
template <typename Scalar>
void symmetricEigh3x3Batched(
...
/// Enqueue the same caller-owned view decomposition; synchronize then check workspace status.
template <typename Scalar>
void symmetricEigh3x3BatchedAsync(
    ConstMatrixView<Scalar, Device::GPU> compact_matrices,
    MatrixView<Scalar, Device::GPU> eigenvalues,
...
void symmetricEigh3x3Batched(
    ...
    MatrixView<Scalar, Device::GPU>, SymmetricEigh3x3Workspace&, cudaStream_t = nullptr)
{
    throw std::runtime_error(
        "symmetricEigh3x3Batched GPU view overload requires PLAMATRIX_WITH_CUDA=ON");
}
...
{
    throw std::runtime_error(
        "symmetricEigh3x3BatchedAsync GPU view overload requires PLAMATRIX_WITH_CUDA=ON");
}

...
/// Symmetric eigendecomposition via cuSOLVER. Eigenvalues are ascending and eigenvectors are columns.
template <typename Scalar>
std::tuple<DenseStorage<Scalar, Device::GPU>, DenseStorage<Scalar, Device::GPU>>
eighDecomposition(const DenseStorage<Scalar, Device::GPU>& A);

/// Cholesky factorization through cuSOLVER. The lower triangle contains L for A = L*L^T.
template <typename Scalar>
DenseStorage<Scalar, Device::GPU> cholesky(const DenseStorage<Scalar, Device::GPU>& A);

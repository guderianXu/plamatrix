...
class CooStorage : public DeviceStorage<Scalar, Dev>
{
public:
    ...
    CooStorage(Index rows, Index cols)
        : DeviceStorage<Scalar, Dev>(0, 0)
    {
        if (rows < 0 || cols < 0)
        {
            throw std::invalid_argument("CooStorage dimensions must be non-negative");
        }
        this->_rows = rows;
        this->_cols = cols;
    }
    ...
    void add(Index row, Index col, Scalar value)
    {
        ...
        {
            throw std::out_of_range("CooStorage triplet index is out of bounds");
        }
    ...
    /// @return CsrStorage with equivalent sparse representation
    CsrStorage<Scalar, Dev> toCsr() const
    {

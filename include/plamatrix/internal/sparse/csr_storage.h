...
class CsrStorage : public DeviceStorage<Scalar, Dev>
{
public:
    ...
    CsrStorage(Index rows, Index cols, Index nnz)
        : CsrStorage(rows, cols, nnz, AllocationKind::Regular, nullptr, true)
    {
    }
    ...
    static CsrStorage pinned(Index rows, Index cols, Index nnz)
    {
#ifdef PLAMATRIX_NO_CUDA
        static_cast<void>(rows);
        static_cast<void>(cols);
        static_cast<void>(nnz);
        throw std::runtime_error("CsrStorage::pinned requires PLAMATRIX_WITH_CUDA=ON");
#else
        return CsrStorage(rows, cols, nnz, AllocationKind::PinnedHost, nullptr, true);
#endif
    }
    ...
    static CsrStorage uninitializedAsync(
        Index rows, Index cols, Index nnz, cudaStream_t stream = nullptr)
    {
        ...
        throw std::runtime_error(
            "CsrStorage::uninitializedAsync requires PLAMATRIX_WITH_CUDA=ON");
#else
        return CsrStorage(
    ...
    // Non-copyable
    CsrStorage(const CsrStorage&) = delete;
    CsrStorage& operator=(const CsrStorage&) = delete;

    /// Move constructor. Transfers ownership of CSR arrays from source.
    /// @param other  Source matrix (left in null state)
    CsrStorage(CsrStorage&& other) noexcept
        : DeviceStorage<Scalar, Dev>(std::move(other))
        , _nnz(other._nnz)
    ...
    CsrStorage& operator=(CsrStorage&& other) noexcept
    {
        if (this != &other)
        {
            releaseArrays();
            DeviceStorage<Scalar, Dev>::operator=(std::move(other));
            _nnz = other._nnz;
            _values = other._values;
            _col_indices = other._col_indices;
            _row_offsets = other._row_offsets;
    ...
    void validateStructure(cudaStream_t stream = nullptr) const
    {
        if (_hasPendingWrite && _pendingWriteStream != stream)
        {
            throw std::logic_error(
                "CsrStorage::validateStructure must use the stream of the pending async copy");
        }
        ...
        else
        {
            ...
            static_cast<void>(stream);
            throw std::runtime_error(
                "CsrStorage::validateStructure requires PLAMATRIX_WITH_CUDA=ON");
#else
    ...
    void closeAsyncAllocation()
    {
        ...
        {
            throw std::logic_error(
                "CsrStorage::closeAsyncAllocation requires stream-ordered storage");
        }
#ifdef PLAMATRIX_NO_CUDA
        throw std::runtime_error(
            "CsrStorage::closeAsyncAllocation requires PLAMATRIX_WITH_CUDA=ON");
#else
    ...
    template <Device D = Dev, std::enable_if_t<D == Dev && D == Device::GPU, int> = 0>
    CsrStorage<Scalar, Device::CPU> toCpu() const
    {
        if (isAsyncAllocation())
        {
            throw std::logic_error(
                "CsrStorage::toCpu cannot synchronously read stream-ordered storage; "
                "use toCpuAsync on the owning stream");
        }
#ifdef PLAMATRIX_NO_CUDA
        throw std::runtime_error("CsrStorage::toCpu requires PLAMATRIX_WITH_CUDA=ON");
#else
        if (_hasPendingWrite)
        {
        ...
        CsrStorage<Scalar, Device::CPU> result(this->_rows, this->_cols, _nnz);
        if (_nnz > 0)
        {
    ...
    /// @throws std::overflow_error if a transfer byte count overflows size_t
    template <Device D = Dev, std::enable_if_t<D == Dev && D == Device::GPU, int> = 0>
    CsrStorage<Scalar, Device::CPU> toCpuAsync(cudaStream_t stream = nullptr) const
    {
#ifdef PLAMATRIX_NO_CUDA
        static_cast<void>(stream);
        throw std::runtime_error("CsrStorage::toCpuAsync requires PLAMATRIX_WITH_CUDA=ON");
#else
        checkAsyncStream("toCpuAsync", stream);
        auto result = CsrStorage<Scalar, Device::CPU>::pinned(
    ...
    template <Device D = Dev, std::enable_if_t<D == Dev && D == Device::GPU, int> = 0>
    void copyToCpuAsync(
        CsrStorage<Scalar, Device::CPU>& output, cudaStream_t stream = nullptr) const
    {
        checkTransferOutput("copyToCpuAsync", output.rows(), output.cols(), output.nnz());
#ifdef PLAMATRIX_NO_CUDA
        static_cast<void>(stream);
        throw std::runtime_error("CsrStorage::copyToCpuAsync requires PLAMATRIX_WITH_CUDA=ON");
#else
        ...
        if (!output.isPinnedHost())
        {
            throw std::invalid_argument(
                "CsrStorage::copyToCpuAsync requires pinned CPU output storage");
        }
    ...
    /// @throws std::overflow_error if a transfer byte count overflows size_t
    template <Device D = Dev, std::enable_if_t<D == Dev && D == Device::CPU, int> = 0>
    CsrStorage<Scalar, Device::GPU> toGpu() const
    {
#ifdef PLAMATRIX_NO_CUDA
        throw std::runtime_error("CsrStorage::toGpu requires PLAMATRIX_WITH_CUDA=ON");
#else
        validateStructure();
        CsrStorage<Scalar, Device::GPU> result(this->_rows, this->_cols, _nnz);
        if (_nnz > 0)
        {
            PLAMATRIX_CHECK_CUDA(cudaMemcpy(
    ...
    /// @throws std::overflow_error if a transfer byte count overflows size_t
    template <Device D = Dev, std::enable_if_t<D == Dev && D == Device::CPU, int> = 0>
    CsrStorage<Scalar, Device::GPU> toGpuAsync(cudaStream_t stream = nullptr) const
    {
#ifdef PLAMATRIX_NO_CUDA
        static_cast<void>(stream);
        throw std::runtime_error("CsrStorage::toGpuAsync requires PLAMATRIX_WITH_CUDA=ON");
#else
        if (!isPinnedHost())
        {
            throw std::invalid_argument(
                "CsrStorage::toGpuAsync requires pinned CPU input storage");
        }
        validateStructure();
        auto result = CsrStorage<Scalar, Device::GPU>::uninitializedAsync(
    ...
    template <Device D = Dev, std::enable_if_t<D == Dev && D == Device::CPU, int> = 0>
    void copyToGpuAsync(
        CsrStorage<Scalar, Device::GPU>& output, cudaStream_t stream = nullptr) const
    {
        checkTransferOutput("copyToGpuAsync", output.rows(), output.cols(), output.nnz());
#ifdef PLAMATRIX_NO_CUDA
        static_cast<void>(stream);
        throw std::runtime_error("CsrStorage::copyToGpuAsync requires PLAMATRIX_WITH_CUDA=ON");
#else
        if (!isPinnedHost())
        {
            throw std::invalid_argument(
                "CsrStorage::copyToGpuAsync requires pinned CPU input storage");
        }
        validateStructure();
...
private:
        ...
        , _mutablePointersEscaped(false)
    {
        if (rows < 0 || cols < 0)
        {
            throw std::invalid_argument("CsrStorage dimensions must be non-negative");
        }
        if (nnz < 0)
        {
            throw std::invalid_argument("CsrStorage nnz must be non-negative");
        }
        if (nnz > 0 && (rows == 0 || cols == 0))
        {
            throw std::invalid_argument(
                "CsrStorage cannot store non-zero entries with zero rows or columns");
        }
        this->_rows = rows;
        this->_cols = cols;
    ...
    void validateHostStructure() const
    {
        ...
        {
            throw std::invalid_argument(
                "CsrStorage row offsets must start at zero and end at nnz");
        }
        for (Index row = 0; row < this->_rows; ++row)
        {
            ...
            if (begin < 0 || end < begin || end > _nnz)
            {
                ...
                message << "CsrStorage invalid row offsets at row " << row;
        ...
        for (Index position = 0; position < _nnz; ++position)
        {
            ...
            {
                std::ostringstream message;
                message << "CsrStorage column index out of range at position " << position;
                throw std::invalid_argument(message.str());
            }
            if (!std::isfinite(static_cast<double>(_values[position])))
            {
                std::ostringstream message;
                message << "CsrStorage value is not finite at position " << position;
                throw std::invalid_argument(message.str());
            }
        }
    }
    ...
    void checkTransferOutput(const char* operation, Index rows, Index cols, Index nnz) const
    {
        ...
        {
            std::ostringstream message;
            message << "CsrStorage::" << operation << " output mismatch: output is "
...
{
    template <typename Scalar, Device Dev>
    static Scalar* values(CsrStorage<Scalar, Dev>& matrix) noexcept
    {
        return matrix._values;
    }

    template <typename Scalar, Device Dev>
    static Index* colIndices(CsrStorage<Scalar, Dev>& matrix) noexcept
    {
        return matrix._col_indices;
    }

    template <typename Scalar, Device Dev>
    static Index* rowOffsets(CsrStorage<Scalar, Dev>& matrix) noexcept
    {
    ...
    template <typename Scalar, Device Dev>
    static void markAsyncWriteTrusted(CsrStorage<Scalar, Dev>& matrix) noexcept
    {
        matrix.markAsyncWriteTrusted();
    }

    template <typename Scalar, Device Dev>
    static void completeAsyncWrite(CsrStorage<Scalar, Dev>& matrix, bool trusted) noexcept
    {

...
public:
    ...
    DeviceStorage(Index rows, Index cols)
        : _rows(rows)
        , _cols(cols)
        , _data(nullptr)
    ...
    DeviceStorage(DeviceStorage&& other) noexcept
        : _rows(other._rows)
        , _cols(other._cols)
        , _data(other._data)
    ...
    void closeAsyncAllocation()
    {
        ...
        if constexpr (Dev == Device::CPU)
        {
            throw std::logic_error(
                "DeviceStorage::closeAsyncAllocation requires a stream-ordered GPU allocation");
        }
        else
        {
...
protected:
    DeviceStorage(Index rows, Index cols, detail::HostAllocationKind host_allocation_kind)
        : _rows(rows)
        , _cols(cols)
        , _data(nullptr)
    ...
    DeviceStorage(Index rows, Index cols, detail::AsyncGpuAllocationTag, cudaStream_t stream)
        : _rows(rows)
        , _cols(cols)
        , _data(nullptr)
        , _host_allocation_kind(detail::HostAllocationKind::Pageable)
    ...
    static std::size_t checkedElementCount(Index rows, Index cols)
    {
        if (rows < 0 || cols < 0)
        {
            throw std::invalid_argument("DeviceStorage dimensions must be non-negative");
        }
        if (rows == 0 || cols == 0)
        {
            return 0;
        ...
        {
            throw std::overflow_error("DeviceStorage element count overflows Index");
        }

...
- **CUDA GPU**：`cublasSgemm` / `cublasDgemm`，支持指定 CUDA stream
- **Metal GPU**：`float` 使用 `MPSMatrixMultiplication`；`double` 使用 CPU/共享内存 fallback
- **输出复用**：`gemm(A_gpu, B_gpu, C_gpu, stream)` 写入已有输出矩阵，适合循环里避免反复分配
- **异步接口**：CUDA 后端只提交 cuBLAS 工作，不主动同步；Metal 后端目前忽略 `cudaStream_t` 兼容参数并在返回前完成
- **示例**：`auto C = gemm(A, B);`
...
- GPU 接口使用自定义 CUDA kernel 或 Metal compute kernel；Metal 后端目前忽略 `cudaStream_t` 兼容参数
...
- **CUDA GPU**：`cusolverDnSgesvd` (float) / `cusolverDnDgesvd` (double)，legacy cuSOLVER 路径要求 `rows >= cols`
- **Metal GPU**：首版使用 CPU fallback 后转回 `Device::GPU` 矩阵，保持 API 兼容
...
- **CUDA GPU**：`cusolverDnSgeqrf` + `cusolverDnSorgqr`
- **Metal GPU**：首版使用 CPU fallback 后转回 `Device::GPU` 矩阵
...
- **CUDA GPU**：`cusolverDnSsyevd` (分治算法)
- **Metal GPU**：首版使用 CPU fallback 后转回 `Device::GPU` 矩阵
...
- **CPU**：列主元高斯消元 (LU 分解)
- **CUDA GPU**：`cusolverDnSgetrf` + `cusolverDnSgetrs`
- **Metal GPU**：`float` 使用 MPS LU；`double` 使用 CPU fallback 后转回 `Device::GPU` 矩阵
...
| 运算 | GPU 临界尺寸 | 说明 |
|------|-------------|------|
| gemm | 依 BLAS/CUDA/Metal 和矩阵尺寸而定 | CPU BLAS 在小矩阵上可能优于 GPU |
| add/sub | ~4096 | 内存带宽受限，GPU 优势出现较晚 |
| svd | 依 LAPACK/CUDA 和矩阵尺寸而定 | Metal 首版是 CPU fallback |
| solve | ~256 | GPU LU 分解远超 CPU 高斯消元 |

*注：临界尺寸因硬件而异，请使用 `plamatrix_benchmark` 工具实际测量。*

GPU benchmark 会按当前编译后端输出 `GPU cuda (ms)` 或 `GPU metal (ms)` 报告列。
CUDA 后端在 GPU run 边界启用 memory pool，GEMM/add/sub 使用 CUDA event 计时并复用输出矩阵。
Metal 后端 benchmark 覆盖 GEMM、add/sub、transpose、solve 和 point transform；SVD/QR/Eigh/covariance 当前 fallback-heavy，不作为 Metal GPU 性能结果。
